...
import * as inv from '../services/inventory/inventoryOverviewService.js';
...
router.get('/businesses/:businessId/inventory-overview', async (req, res, next) => {
  try { res.json(await inv.getOverview(db, ctxFromReq(req))); }
  catch (e) { next(e); }
});
